...
Токен передаётся в заголовке `Authorization: Bearer <token>`. На фронтенде он
сохраняется в `localStorage` и автоматически подставляется axios-интерцептором.
Если пользователь авторизован, создаваемые проекты и задачи привязываются к нему.
...
curl -X POST http://localhost:4000/api/auth/register \
  ...
  -d '{"email": "user@example.com", "password": "secret123", "firstName": "Иван", "lastName": "Петров"}'
```
